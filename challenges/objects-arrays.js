...
console.log(tyrannosaurus.peroid);
...
zooAnimals.forEach(element => {
  let twoNames = `Name: ${element.animal_name}, ${element.scientific_name}`
  return animalNames.push(twoNames)
})
...
const lowerCase = [];
zooAnimals.map(item => {
  let firstName = `Name: ${item.animal_name}`
  return lowerCase.push(firstName)
})
